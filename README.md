...
通常编译过程不会出现问题，如何出现任何问题，请联系作者，并将完整的错误信息
保留并报告给作者。
...
除--config参数之外，所有参数均可在命令行或配置文件中，在下
面一一解释各参数相关作用
...
```
--data_shards
--parity_shards	这2个参数是fec参数，主要指定多少份data_shards和多少份parity_shards组成一组fec数据，也就是说，在发送data_shards份数据的同时，携带parity_shards份冗余数据。fec算法可以允许任意最多丢失parity_shards份数据。
```
```
