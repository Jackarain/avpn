...
namespace avpn
{
	channel::channel(boost::asio::io_context& io, io_context_pool& ios, const channel_params& params, avpn_service& service)
	...
	void channel::start_listen(std::vector<std::string> tcp_listens, std::vector<std::string> udp_listens)
	{
		...
		if (m_params.data_shards_ > 1)
		{
			...
			boost::asio::co_spawn(m_main_ioc.get_executor(),
				run_fec_dispatch(), boost::asio::detached);
		}
	...
	void channel::start_connect(const std::vector<std::string>& upstreams)
	{
		...
		boost::asio::co_spawn(m_main_ioc.get_executor(),
			...
			{
				co_return co_await start_tcp_connect();
			}, boost::asio::detached);
	}
	...
	void channel::server_forward_tun(vpn_message&& msg, endpoint_pair&& endp)
	{
	...
	void channel::client_forward_tun(vpn_message&& msg, endpoint_pair&& endp)
	{
		auto connection_ptr = m_client.lock();
		if (!connection_ptr)
		{
	...
	boost::asio::ip::network_v4 channel::vnet_ipaddr() const
	{
		return m_vnet_ipaddr;
	}

	boost::asio::ip::network_v4 channel::vnet() const
	{
		// 总是使用网络中第1个地址作为网关.
	...
	bool channel::init_tcp_acceptors()
	{
	...
	boost::asio::awaitable<void> channel::start_ws_listen(tcp::acceptor& a)
	{
	...
	boost::asio::awaitable<void> channel::start_ws_connection(size_t connection_id, boost::beast::tcp_stream stream)
	{
	...
	void channel::keepalive(vpn_connection_weak_ptr ptr)
	{
	...
	void channel::ws_expires_after(vpn_connection& connection, int seconds)
	{
		// 设置超时.
	...
	boost::asio::awaitable<void> channel::start_tcp_read(vpn_connection_ptr connection_ptr)
	{
	...
	boost::asio::awaitable<void> channel::process_tcp_packet(uint32_t type,
		stream_endian::bitstream& reader, vpn_connection_ptr& connection_ptr)
	{
		...
		{
		case vpt_auth:
			co_return co_await do_vpt_auth(reader, connection_ptr);
			break;
		...
		case vpt_udp:
			[[fallthrough]];
		case vpt_tcp:
			co_return co_await do_vpt_packet(type, reader, nullptr);
		case vpt_fec:
	...
	boost::asio::awaitable<void> channel::process_udp_packet(uint8_t type,
		stream_endian::bitstream& reader, udp::socket& sock, const udp::endpoint& endp)
	{
		...
		case vpt_keepalive:
			break;
		case vpt_udp_handshake:
			co_return co_await do_vpt_udp_handshake(reader, sock, endp);
		case vpt_udp_handshake_reply:
			co_return co_await do_vpt_udp_handshake_reply(reader, endp);
		case vpt_compress_tcp:
			[[fallthrough]];
		case vpt_compress_udp:
			{
		...
		case vpt_udp:
			[[fallthrough]];
		case vpt_tcp:
			co_return co_await do_vpt_packet(type, reader, &endp);
	...
	boost::asio::awaitable<void> channel::forward_tcp_write(const vpn_connection_ptr& connection_ptr, std::string&& msg)
	{
		auto& connection = *connection_ptr;
	...
	boost::asio::awaitable<void> channel::forward_udp_write(
		udp::socket& sock, const udp::endpoint& endp, std::string&& msg)
	{
		boost::system::error_code ec;
		co_await sock.async_send_to(boost::asio::buffer(msg), endp, uawaitable[ec]);
		if (ec)
			LOG_DBG << "forward_udp_write, async_send_to error: " << ec.message();

		co_return;
	}

	boost::asio::awaitable<void> channel::forward_channel_write(vpn_connection_ptr connection_ptr, avpn::vpn_message&& msg)
	{
		auto& connection = *connection_ptr;
		connection.deque_writing_ = !connection.ws_msg_deque_.empty();
	...
	boost::asio::awaitable<void> channel::start_udp_socket()
	{
		boost::system::error_code ec;
		...
		if (m_identity == avpn_server)
		{
			...
			for (auto& listen : m_udp_listens)
			{
				...
				m_udp_sockets.emplace_back(udp_socket{ time_clock::steady_clock::now(), std::move(sock) });
			}
		}
		else
		{
			...
			auto size = m_remote_endps.size();
			for (auto i = 0; i < size; i++)
			{
				auto endp = m_remote_endps.acquire();

				// 每个上游服务端, 都开启max_client_udp_socket个socket同时通信.
				for (int i = 0; i < max_client_udp_socket; i++)
				{
					udp::socket sock(m_main_ioc, udp::endpoint(endp.protocol(), 0));
					if (ec)
					{
						LOG_ERR << "Start client udp open error: " << ec.message();
						continue;
					}
					...
					if (!connection_ptr)
					{
						LOG_ERR << "Start client udp, connection is nullptr!";
						co_return;
					}
	...
	boost::asio::awaitable<void> channel::start_udp_read_loop(size_t index)
	{
		auto& udp_sock = m_udp_sockets[index];
	...
	boost::asio::awaitable<void> channel::start_tcp_connect()
	{
		boost::system::error_code ec;
		...
		while (!m_abort)
		{
				...
				it != m_upstreams.end() && !m_abort; it++)
			{
				...
				if (ec)
				{
					LOG_ERR << "start_tcp_connect, find udp async_resolve: " << ec.message();
					continue;
				}
			...
			LOG_DBG << "start_tcp_connect, connect to: " << remote_host;
	...
	void channel::add_connection(vpn_connection_ptr& connection_ptr, uint32_t vaddr)
	{
		std::lock_guard<std::mutex> lock(m_server_mtx);
		m_remotes[vaddr] = connection_ptr;
	}

	void channel::remove_connection(uint32_t vaddr)
	{
		std::lock_guard<std::mutex> lock(m_server_mtx);
		m_remotes.erase(vaddr);
	}

	vpn_connection_ptr channel::lookup_connection(uint32_t vaddr)
	{
		std::lock_guard<std::mutex> lock(m_server_mtx);
	...
	void channel::add_incoming(vpn_connection_ptr& connection_ptr, int64_t id)
	{
		std::lock_guard<std::mutex> lock(m_server_mtx);
		m_incomings[id] = connection_ptr;
	}

	void channel::remove_incoming(int64_t id)
	{
		std::lock_guard<std::mutex> lock(m_server_mtx);
		m_incomings.erase(id);
	}

	boost::asio::awaitable<void> channel::http_response_handle(
	...
	std::tuple<std::string, uint32_t> channel::ip_assigner()
	{
	...
	boost::asio::awaitable<void> channel::do_vpt_auth(
		stream_endian::bitstream& reader, vpn_connection_ptr& connection_ptr)
	{
		auto& connection = *connection_ptr;
	...
	boost::asio::awaitable<bool> channel::do_vpt_compress(
	...
	boost::asio::awaitable<void> channel::do_vpt_packet(uint8_t type,
	...
	boost::asio::awaitable<vpn_connection_ptr> channel::do_vpt_fec_packet(
	...
	boost::asio::awaitable<void> channel::do_vpt_udp_handshake(
		stream_endian::bitstream& reader, udp::socket& sock, const udp::endpoint& endp)
	{
		// 协议格式:
		// type(number)
		...
		if (m_identity == avpn_server)
		{
			...
			co_return co_await forward_udp_write(sock, endp, std::move(reply));
		}
	...
	boost::asio::awaitable<void> channel::do_vpt_udp_handshake_reply(
		stream_endian::bitstream& reader, const udp::endpoint& endp)
	{
		// 协议格式:
		// type(number)
	...
	boost::asio::awaitable<void> channel::do_fec_process(vpn_connection_ptr& connection_ptr)
	{
		auto& connection = *connection_ptr;
	...
	boost::asio::awaitable<void> channel::run_fec_dispatch()
	{
		if (m_params.mode_ == vpn_tcp_mode::only_tcp)
			co_return;
	...
	boost::asio::awaitable<bool> channel::do_fec_perform(vpn_connection_ptr& connection_ptr)
	{
		// 继续发送.
		bool keep_continue = false;
			...
			&& m_params.mode_ != vpn_tcp_mode::only_udp)
		{
			for (auto& msg : fec_enc)
			{
				...
				boost::asio::co_spawn(m_main_ioc,
					forward_tcp_write(connection_ptr, std::move(pkt)), boost::asio::detached);
			}
