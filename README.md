...
Android 真机对比（Pixel 6 经 Wi-Fi ↔ 2 vCPU VPS，RTT ≈ 140ms，MTU
1400，8 条并发下行 TCP 流，单次 10–14s，取同一时间窗交替测量）：

| 方案 | 下行吞吐 | 网关侧 CPU |
| --- | --- | --- |
| avpn，UDP + FEC 8/4 | ~40 Mbit/s | ~80% 单核 |
| avpn，UDP + FEC 关闭 | ~42 Mbit/s | ~70% 单核 |
| avpn，TCP + 批量读写 | ~34–46 Mbit/s | ~45–50% 单核 |
| xProxy（TLS/TCP 代理） | ~69–76 Mbit/s | ~7% 单核 |

同一链路上 avpn 每字节消耗的网关 CPU 约为 xProxy 的 4 倍：xProxy 服务端
只做 TCP 中继，数据不经过 tun 与逐包加密；avpn 每个内层报文都要经过
tun 读写、AEAD 与组帧，再用系统调用逐包发出。因此把外层发送改成 TSO
批量提交只是减少了发送侧开销，吞吐仍受限于隧道自身的每包处理。同向
继续提升需要传输并行化（多连接/多线程）而不是继续压缩单包开销。

### 千兆内网调优结果

在千兆内网（本机 ↔ 192.168.1.143，MTU 1448，FEC 8/0，8 流 TCP，20s）上，
隧道吞吐最终达到物理链路的 96.6%（下行）/94.0%（上行）：物理
856.0/895.4 Mbit/s，隧道 827.0/841.4 Mbit/s。本轮为高速率链路做的改动：
...
优化前的同链路记录（MTU 1500）为隧道下行 837 Mbit/s（物理 941 Mbit/s，
88.9%）、上行 909 Mbit/s（物理 952 Mbit/s，95.5%），可见下行受 queue/qdisc
丢包影响的部分已补齐；上行本已接近物理上限，剩余差距来自内层 ACK 与外层
小包的每包开销。

Android 端（Pixel 6 经 Wi-Fi ↔ VPS，单程 RTT 约 180ms，链路本身波动较大）
按 16 流 TCP/12s 与物理直连对比，同一时段交替测量：

| 路径 | 上行 | 下行 |
| --- | --- | --- |
| 物理直连（不经隧道） | 30.9 Mbit/s | 91.5 Mbit/s |
| avpn 隧道 | 37.0 Mbit/s | 106.7 Mbit/s |
| xProxy | 38.7 Mbit/s | 118.0 Mbit/s |

avpn 上/下行分别为 xProxy 的 95.5%/90.4%，且都高于同条件下物理直连的 TCP
吞吐。并发流数对结论影响极大：8 流时 avpn 上行只有 26 Mbit/s，16 流时升到
37 Mbit/s，原因是 ttcp 单连接吞吐受自身发送缓冲与 180ms RTT 限制（约
3.2 Mbit/s/流），低流数下测到的是测量工具上限而不是隧道上限；xProxy 属应用
层代理，内层 TCP 在手机侧终结（外层为 8 条 TLS/TCP 连接），不受该限制，因
此低流数下其优势被放大。下行 98–107 Mbit/s 时 avpn 手机侧数据面线程约 72%
单核、网关侧约 52% 单核，两端均未饱和，剩余 5–10% 差距来自传输形态（UDP
隧道逐包处理、外层无聚合）而不是算力。
