...
//
// launcher 入口：创建/启停多个 avpn 进程实例，并通过
// WebSocket + JSON-RPC 实现运行期状态展示与配置管理。
//
...
int main(int argc, char** argv)
{
	...
	desc.add_options()
		...
		("ssl_certificate_dir", po::value<std::string>(&ssl_dir)
			->value_name("path"),
			"Directory containing SSL/TLS certificates for HTTPS WebUI (auto-search).")
		("avpn", po::value<std::string>(&avpn_path)
			->default_value("./avpn")->value_name("path"),
			"Path to the avpn executable.")
		...
		("no_kill_on_exit", po::value<bool>(&no_kill_on_exit)
			...
			"Do not stop avpn instances when launcher exits.")
		;
	...
	// 解析 avpn 可执行文件路径。
	// 未显式指定 --avpn 时：优先当前目录下的 avpn，其次在系统 $PATH 中查找。
	if (vm["avpn"].defaulted()) {
		boost::system::error_code ec;
		if (!fs::exists(avpn_path, ec)) {
			std::string found = find_in_path("avpn");
			if (!found.empty())
				avpn_path = std::move(found);
		}
	}
	...
	// 检查 avpn 可执行文件是否存在。
	{
		boost::system::error_code ec;
		if (!fs::exists(avpn_path, ec)) {
			std::fprintf(stderr, "\x1b[31m[错误] 未找到 avpn 可执行文件: %s\x1b[0m\n", avpn_path.c_str());
			std::fprintf(stderr, "请将 avpn 放到 launcher 的当前目录或系统 $PATH 中，或使用 --avpn 指定其路径。\n");
			return 1;
		}
	}
	...
	// 实例管理器。
	auto mgr = std::make_shared<manager>(data_dir, avpn_path, work_dir.string());
