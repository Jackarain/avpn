...
import 'package:xavpn/services/update_service.dart';

/// 发布目录列表的真实响应形态.
const String _indexJson = '''
[
  {"last_write_time":"09-25-2026 00:20","filename":"avpn-release.apk","is_dir":false,
   "filesize":71629274,"hash":"B2DDD179B8837A234D895D346ABA0E52691A2452"},
...
const String _apkSha1 = 'b2ddd179b8837a234d895d346aba0e52691a2452';
...
void main() {
  ...
  setUp(() async {
    temp = await Directory.systemTemp.createTemp('xavpn_update_test');
  });
  ...
  group('fetch', () {
    test('从列表里取安装包的 SHA-1 与大小', () async {
      ...
      expect(remote.size, 71629274);
      expect(remote.lastWriteTime, '09-25-2026 00:20');
    });
    ...
    test('校验值缺失或格式不对时报错', () async {
      for (final hash in ['', 'abc', 'z' * 40, 'a' * 64]) {
        final server = await _serveIndex(
          jsonEncode([
            {'filename': 'avpn-release.apk', 'filesize': 1, 'hash': hash},
          ]),
        );
        addTearDown(() => server.close(force: true));
        expect(
  ...
  group('download', () {
    test('写出文件并按数据块上报进度', () async {
      ...
      final received = await UpdateDownload(
        url: _urlOf(server, '/avpn-release.apk'),
        target: target,
