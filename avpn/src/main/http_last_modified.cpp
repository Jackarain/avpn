...
//
// http_last_modified.cpp is based on lib/parsedate.c from curl copyright.
// http_last_modified.cpp is based on src/http/ngx_http_parse_time.c from nginx copyright.
...
#  endif
	...
	const static uint64_t  mday[] = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };

	time_t parse_http_time(const char* value, size_t len)
	{
		const uint8_t* p, * end;
		int64_t    month = 0;
		uint64_t   day = 0, year = 0, hour = 0, min = 0, sec = 0;
		uint64_t     time = 0;
		enum {
			no = 0,
		...
		fmt = no;
		end = (const uint8_t*)value + len;

		for (p = (const uint8_t*)value; p < end; p++) {
			if (*p == ',') {
				break;
			}
		...
		if (fmt == rfc822) {
			...
			year = (*p - (uint64_t)'0') * 1000 + (*(p + 1) - (uint64_t)'0') * 100
				+ (*(p + 2) - (uint64_t)'0') * 10 + (*(p + 3) - '0');
			p += 4;
		...
		min = (*p - (uint64_t)'0') * 10 + (*(p + 1) - (uint64_t)'0');
		p += 2;
		...
		sec = (*p - (uint64_t)'0') * 10 + (*(p + 1) - (uint64_t)'0');
		...
		if (fmt == isoc) {
			...
			year = (*p - (uint64_t)'0') * 1000 + (*(p + 1) - (uint64_t)'0') * 100
				+ (*(p + 2) - (uint64_t)'0') * 10 + (*(p + 3) - (uint64_t)'0');
		}
...
#if defined(STRICT_HTTP_TIME_PARSER)
		if (parsedate(gmttime.c_str(), &out) != 0)
			return -1;
#else
		out = parse_http_time(gmttime.c_str(), gmttime.size());
#endif
