...
namespace avpn {
	...
	vpn_packet make_error(uint32_t src,
		uint32_t error_code, std::string_view msg)
	{
		auto pkt = make_common_header(
			false, vpt_error, src);
		...
		writer.WriteUInt16((uint16_t)msg.size());
		writer.WriteString(msg.data(), msg.size());
		...
		uint32_t& error_code, std::string& msg)
	{
		...
		auto ret = reader.ReadUInt32(&error_code);
		if (!ret) return -1;
		...
		msg.resize(length);
		ret = reader.ReadString((char*)msg.data(), msg.size());
		if (!ret) return -1;
