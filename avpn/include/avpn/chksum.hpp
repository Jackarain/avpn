...
namespace avpn {
	...
	inline uint32_t inet_cksum_pseudo_base(
		const uint8_t* buf, uint16_t type, int len, uint32_t acc)
	{
		int swapped = 0;
	...
	inline uint16_t tcp_chksum_pseudo(
	...
	inline uint16_t udp_chksum_pseudo(
		const uint8_t* buf, int len, const endpoint_pair& endp)
	{
		uint32_t acc;
		uint32_t addr;
