...
hash256 hash_result(const hash256& header_hash, const hash256& mix_hash, uint64_t nonce) noexcept
{
	const hash512 seed = hash_seed(header_hash, nonce);
	return hash_final(seed, mix_hash);
}

bool light_verify(const hash256& hash, const hash256& boundary) noexcept
{
	return is_less_or_equal(hash, boundary);
}
