...
{
public:
	manager(std::string data_dir, std::string avpn_path, std::string work_dir);
	...
	// 保存新配置；在线实例的修改需重启生效（avpn 不支持热改配置），
	// result 返回 {applied, needs_restart, errors} 供 WebUI 展示。
	...
	// 包装为 jsonrpc_session 传入）：关闭旧连接、存入实例、标记在线。
